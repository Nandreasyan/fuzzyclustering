...
def skills_gen_fz(skills_sets, N, min_skill_sets, max_skill_sets, min_edits, max_edits):
    ...
    skills_lv_higher = [0.6, 0.8, 1]

    for _ in range(N):
        user_skills_fz = np.zeros((len(all_skills)), dtype=float)
        ...
        for s in skills_sets_indices:
            ...
            for skill in skill_set:
                i = all_skills.index(skill)
                user_skills_fz[i] = np.random.choice(skills_lv_higher)

        nb_edits = np.random.randint(min_edits, max_edits)
        for _ in range(nb_edits):

            nonzero_idxs = np.where(user_skills_fz != 0)[0]

            notmodifiedindices = set(range(len(all_skills))) - set(nonzero_idxs)

            notmodifiedindices = list(notmodifiedindices)

            i = np.random.choice(list(notmodifiedindices))

            user_skills_fz[i] = np.random.choice(skills_lv_higher)

        users_skills_fz.append(user_skills_fz)
        clusters_ground_truth.append(skills_sets_indices[0])
